  ...
  return (
    <div key={project.id} className='bg-white border-neutral-200 p-4'>
      <div className='flex flex-row justify-between'>
        ...
        <div className='font-semibold text-[12px]'>{new Date(project.updated_at).getFullYear()}</div>
      </div>
      ...
      <div className='flex flex-row justify-between items-center'>
        ...
        >
          <FiChevronDown className={cn('transition-transform duration-300', showReadme ? 'rotate-180' : '')} />
          Preview
        </div>
      </div>
      {showReadme && (
